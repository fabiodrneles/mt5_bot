...
from mt5bot.engine import indicators
...
def test_calculate_rvol_real_volume():
    ...
    df = pd.DataFrame(all_rates, columns=['time', 'open', 'high', 'low', 'close', 'tick_volume', 'real_volume', 'spread'])
    rvol_ratio, current_vol, avg_vol = indicators.calculate_rvol(df, lookback=20)
...
def test_check_setup_93_buy():
    ...
    from mt5bot import legacy_indicators
    is_93_buy = legacy_indicators.check_setup_93_buy(all_rates, ema9_values)
